...
app.post('/api/games', authenticateToken, async (req, res) => {
    ...
    try {
        const insertQuery = `
    INSERT INTO scores (user_id, score, game_name, date)
    VALUES ($1, $2, $3, $4)
    RETURNING *;
  `;
        const values = [userId, score, gameName, new Date()];
        const result = await client.query(insertQuery, values);
        console.log('Score opgeslagen:', result.rows[0]);
    } catch (error) {
...
// Deze middle ware was ook dubbel wat errors gaf
...
// API to fetch user data (protected route) is ook dubbel wat errors geeft
